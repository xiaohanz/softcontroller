...
//    private IfIptoHost hostTracker;
    ...
    public String toString() {
        return "FirewallRule [status=" + status + ",InstallHw=" + InstallHw + ", name=" + name
                ...
                + ", nwDst=" + nwDst + ", protocol=" + protocol + ", tpSrc="
                + tpSrc + ", tpDst=" + tpDst + ", action=" + action + "]";
    }
    @Override
    public int hashCode() {
        final int prime = 31;
...
//    public Status validate(IContainer container) {
        ...
        try {
            if (ingressPort != null) {
                ...
                if (isPortValid(port) == false) {
                    String msg = String.format("Ingress port %d is not valid for Switch", port);
                    return new Status(StatusCode.BADREQUEST, msg);
                }
            }
            if (name == null || name.trim().isEmpty() || !name.matches(FirewallRule.NAMEREGEX)) {
                return new Status(StatusCode.BADREQUEST, "Invalid name");
            }
            if (priority == null) {
                return new Status(StatusCode.BADREQUEST, String.format("priority is null"));
            }
            ...
            if (etherType==null){
                if(nwSrc!=null||nwDst!=null){
                    return new Status(StatusCode.BADREQUEST,String.format("Ethernet type is null or empty with src/dst ip exist"));
                }
            }
            if (etherType != null) {
            ...
            if (protocol==null){
                if(tpSrc!=null||tpDst!=null){
                    return new Status(StatusCode.BADREQUEST,String.format("Protocol is null or empty with src/dst tcp/udp port exist"));
                }
            }
            ...
            if (action == null) {
                ...
                if(action.startsWith("DISPATCH")||action.startsWith("dispatch")){
                    ...
                    if (dst==null){
                        ...
                        if (!NetUtils.isIPv4AddressValid(dst)&&!NetUtils.isIPv6AddressValid(dst)) {
                            return new Status(StatusCode.BADREQUEST, String.format("IP destination address %s is not valid",
                                    dst));
                        }
                    }
    ...
    public boolean equalToRule(Packet pak,NodeConnector nodeConnector) {
        ...
        InetAddress dstIp=NetUtils.getInetAddress(pkt.getDestinationAddress());
        //get connector info
        ...
        if (this.etherType==null||pktRule.getEtherType().equals(this.getEtherTypeName()))
            if (this.nwSrc==null||pktRule.getNwSrc().equals(this.getNwSrc()))
                if(this.nwDst==null||pktRule.getNwDst().equals(this.getNwDst()))
                    if(this.protocol==null||pktRule.getProtocol().equals(this.getProtocolName()))
                        if (this.tpSrc==null||pktRule.getTpSrc().equals(this.getTpSrc()))
                            if (this.tpDst==null||pktRule.getTpDst().equals(this.getTpDst())){
                                return true;
                            }
        return false;
    }
