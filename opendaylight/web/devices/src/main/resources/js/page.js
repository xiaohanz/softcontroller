...
one.f.switchmanager.nodesLearnt = {
  id: {
    ...
    modal: {
      ...
      form: {
        ...
        idleTimeout: "one_f_switchmanager_nodesLearnt_id_modal_form_idleTimeout",
        hardTimeout: "one_f_switchmanager_nodesLearnt_id_modal_form_hardTimeout"
      }
    }
  },
  ...
  ajax : {
    ...
    other : function(url,request, callback) {
  ...
  modal : {
    initialize: {
      updateNode: function(evt) {
        var result = {};
        result['nodeId'] = decodeURIComponent(evt.target.id);
    ...
    body: {
      updateNode: function(nodeId, switchDetails, content) {
        ...
        var $select = one.lib.form.select.create(content.tiers);
        ...
        var $input = one.lib.form.input(content.idleTimeout);
        $input.attr('value', content.idleTimeout);
        $input.attr('id', one.f.switchmanager.nodesLearnt.id.modal.form.idleTimeout);
        $fieldset.append($label).append($input);
                
         // hard timeout
        var $label = one.lib.form.label("Hard Timeout");
        var $input = one.lib.form.input(content.hardTimeout);
        $input.attr('value', content.hardTimeout);
        $input.attr('id', one.f.switchmanager.nodesLearnt.id.modal.form.hardTimeout);
        $fieldset.append($label).append($input);
    ...
    save: function($modal) {
      ...
      result['idleTimeout'] = $('#' + one.f.switchmanager.nodesLearnt.id.modal.form.idleTimeout, $modal).val();
      result['hardTimeout'] = $('#' + one.f.switchmanager.nodesLearnt.id.modal.form.hardTimeout, $modal).val();
