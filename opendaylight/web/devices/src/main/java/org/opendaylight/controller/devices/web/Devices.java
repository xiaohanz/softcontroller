...
public class Devices implements IDaylightWeb {
    ...
    @RequestMapping(value = "/nodeInformation", method = RequestMethod.GET)
    @ResponseBody
    public NodeInformationJsonBean getNodeInformation(@RequestParam("nodeId") String nodeId,HttpServletRequest request, @RequestParam(required = false) String container) {
        short hardTimeout = 0;
        short idleTimeout = 0;
        String containerName = (container == null) ? GlobalConstants.DEFAULT.toString() : container;
        IL2flowmanager l2 = (IL2flowmanager) ServiceHelper.getInstance(IL2flowmanager.class, containerName,this);

        if( l2 != null){
            idleTimeout = l2.getIdleTimeoutByNodeId(nodeId);
            hardTimeout = l2.getHardTimeoutByNodeId(nodeId);
        }

        NodeInformationJsonBean result = new NodeInformationJsonBean();
        result.setHardTimeout(Short.toString(hardTimeout));
        result.setIdleTimeout(Short.toString(idleTimeout));
        result.setTiers(TierHelper.getTiers());
        return result;
    }
    ...
    public StatusJsonBean updateLearntNode(@RequestParam("nodeName") String nodeName,
            ...
            @RequestParam("idleTimeout") String idleTimeout,
            @RequestParam("hardTimeout") String hardTimeout,
        ...
        try {
            ...
            IL2flowmanager l2 = (IL2flowmanager) ServiceHelper.getInstance(IL2flowmanager.class, containerName,this);
            if(l2 != null) {
                l2.setIdleTimeoutByNodeId(nodeId, Short.parseShort(idleTimeout));
                l2.setHardTimeoutByNodeId(nodeId, Short.parseShort(hardTimeout));
            }
        } catch (Exception e) {
            resultBean.setStatus(false);
